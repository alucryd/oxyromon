...
pub async fn update_system_completion(connection: &mut SqliteConnection, id: i64) {
    sqlx::query!(
        "
        UPDATE systems
        SET completion = CASE
...
pub async fn create_game_from_xml(
    ...
    .execute(connection)
    .await
    .unwrap_or_else(|_| panic!("Error while creating game with name '{}'", game_xml.name))
    .last_insert_rowid()
}
...
pub async fn update_games_completion_by_system_id(
    connection: &mut SqliteConnection,
    system_id: i64,
) {
    sqlx::query!(
...
pub async fn update_split_games_completion_by_system_id(
    connection: &mut SqliteConnection,
    system_id: i64,
) {
    sqlx::query!(
...
pub async fn update_non_merged_and_merged_games_completion_by_system_id(
    connection: &mut SqliteConnection,
    system_id: i64,
) {
    sqlx::query!(
...
pub async fn update_jbfolder_games_completion_by_system_id(
    connection: &mut SqliteConnection,
    system_id: i64,
) {
    sqlx::query!(
...
pub async fn create_rom(
    ...
    sqlx::query!(
        "
        INSERT INTO roms (name, size, md5, game_id, parent_id, original)
        VALUES (?, ?, ?, ?, ?, ?)
        ",
        name,
        size,
...
pub async fn create_rom_from_xml(
    ...
    sqlx::query!(
        "
        INSERT INTO roms (name, bios, disk, size, crc, md5, sha1, rom_status, game_id, parent_id, original)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ",
        rom_xml.name,
        bios,
...
pub async fn update_rom(
    ...
    sqlx::query!(
        "
        UPDATE roms
        SET name = ?, size = ?, md5 = ?, game_id = ?, parent_id = ?, original = ?
        WHERE id = ?
        ",
        name,
        size,
...
pub async fn update_rom_from_xml(
    ...
    sqlx::query!(
        "
        UPDATE roms
        SET name = ?, bios = ?, disk = ?, size = ?, crc = ?, md5 = ?, sha1 = ?, rom_status = ?, game_id = ?, parent_id = ?, original = ?
        WHERE id = ?
        ",
        rom_xml.name,
        bios,
...
pub async fn find_original_roms_with_romfile_by_game_ids(
    connection: &mut SqliteConnection,
    game_ids: &[i64],
) -> Vec<Rom> {
    let sql = format!(
...
pub async fn find_roms_without_romfile_by_size_and_md5_and_game_names(
    connection: &mut SqliteConnection,
    size: u64,
    md5: &str,
    game_names: &[&str],
    ...
    sqlx::query_as::<_, Rom>(&sql)
        ...
        .unwrap_or_else(|_| {
            panic!(
                "Error while finding roms with size {} and MD5 {} and game names {:?}",
                size, md5, game_names
            )
        })
}
...
pub async fn find_roms_without_romfile_by_name_and_size_and_md5_and_game_names(
    connection: &mut SqliteConnection,
    name: &str,
    size: u64,
    md5: &str,
    ...
    sqlx::query_as::<_, Rom>(&sql)
        ...
        .unwrap_or_else(|_| {
            panic!(
                "Error while finding roms with name {} and size {} and MD5 {} and game names {:?}",
                name, size, md5, game_names
            )
        })
}
...
pub async fn find_roms_without_romfile_by_size_and_sha1_and_game_names(
    connection: &mut SqliteConnection,
    size: u64,
    sha1: &str,
    game_names: &[&str],
    ...
    sqlx::query_as::<_, Rom>(&sql)
        ...
        .unwrap_or_else(|_| {
            panic!(
                "Error while finding roms with size {} and SHA1 {} and game names {:?}",
                size, sha1, game_names
            )
        })
}
...
pub async fn find_roms_without_romfile_by_name_and_size_and_sha1_and_game_names(
    connection: &mut SqliteConnection,
    name: &str,
    size: u64,
    sha1: &str,
    ...
    sqlx::query_as::<_, Rom>(&sql)
        ...
        .unwrap_or_else(|_| {
            panic!(
                "Error while finding roms with name {} and size {} and SHA1 {} and game names {:?}",
                name, size, sha1, game_names
            )
        })
}
...
pub async fn find_roms_without_romfile_by_size_and_crc_and_game_names(
    connection: &mut SqliteConnection,
    size: u64,
    crc: &str,
    game_names: &[&str],
    ...
    sqlx::query_as::<_, Rom>(&sql)
        ...
        .unwrap_or_else(|_| {
            panic!(
                "Error while finding roms with size {} and CRC {} and game names {:?}",
                size, crc, game_names
            )
        })
}
...
pub async fn find_roms_without_romfile_by_name_and_size_and_crc_and_game_names(
    connection: &mut SqliteConnection,
    name: &str,
    size: u64,
    crc: &str,
    ...
    sqlx::query_as::<_, Rom>(&sql)
        ...
        .unwrap_or_else(|_| {
            panic!(
                "Error while finding roms with name {} and size {} and CRC {} and game names {:?}",
                name, size, crc, game_names
            )
        })
}
